...
#define LOWER_L LT(_LOWER, KC_ENT)
#define LOWER_R LT(_LOWER, KC_ENT)
#define RAISE_L LT(_RAISE, KC_SPC)
#define RAISE_R LT(_RAISE, KC_SPC)
#define CTL_L CTL_T(KC_MHEN)
#define CTL_R CTL_T(KC_HENK)
#define CTL_ML GUI_T(JP_MEISU)
#define CTL_MR GUI_T(JP_MKANA)
...
#define LOWER_ML LT(_MAC_LOWER, KC_ENT)	// タップで無変換     ホールドでLower
#define LOWER_MR LT(_MAC_LOWER, KC_ENT)	// タップで変換       ホールドでRaise
...
[_QWERTY] = LAYOUT( \
  ...
  KC_LSFT, KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,   KC_BSPC,  KC_BSPC, KC_N,    KC_M,    JP_COMM, JP_DOT,  JP_SLSH, SFT_T(JP_BSLS), \
 ...
 */
[_LOWER] = LAYOUT( \
  JP_GRV,       KC_F1,      KC_F2,      KC_F3,      KC_F4,       KC_F5,                       KC_F6,       KC_F7,   KC_F8,         KC_F9,   KC_F10,  JP_MINS, \
  LCTL(KC_TAB), KC_F11,     KC_F12,     KC_END,     LCTL(KC_R),  LCTL(KC_T),                  LCTL(KC_Y),  XXXXXXX, XXXXXXX,       MC_ENTO, KC_UP,   JP_LBRC, \
  _______,      KC_HOME,    LCTL(KC_S), KC_DEL,     KC_RGHT,     LCTL(KC_G),                  KC_BSPC,     XXXXXXX, MC_DELK,       XXXXXXX, JP_SCLN, JP_QUOT, \
  _______,      LCTL(KC_Z), LCTL(KC_X), LCTL(KC_C), LCTL(KC_V),  KC_LEFT,  KC_DEL,   KC_DEL,  KC_DOWN,     XXXXXXX, LCTL(KC_COMMA),LCTL(KC_DOT),JP_SLSH, JP_YEN, \
...
[_RAISE] = LAYOUT( \
  ...
  _______,   XXXXXXX,    XXXXXXX,    XXXXXXX,    XXXXXXX,   MC_CLEFT, MC_BCSP,  MC_BCSP, KC_PGDN, XXXXXXX, JP_LT,   JP_GT,   JP_QUES, JP_PIPE, \
                          _______,    _______,    _______,  _______,  _______,  _______, _______, _______, _______, _______\
),
...
[_MAC_LOWER] = LAYOUT( \
  JP_GRV,       KC_F1,      KC_F2,       KC_F3,      KC_F4,      KC_F5,                       KC_F6,             KC_F7,   KC_F8,   KC_F9,   KC_F10,  JP_MINS, \
  LCTL(KC_TAB), KC_F11,     KC_F12,      LCTL(KC_E), LGUI(KC_R), LGUI(KC_T),                  LGUI(LSFT(KC_Y)),  XXXXXXX, XXXXXXX, MC_ENTO, KC_UP,   JP_LBRC, \
  _______,      LCTL(KC_A),LGUI(KC_S),   KC_DEL,     KC_RGHT,    LGUI(KC_G),                  KC_BSPC,           XXXXXXX, MC_DELK, XXXXXXX, JP_SCLN, JP_QUOT, \
  _______,      LGUI(KC_Z), LGUI(KC_X),  LGUI(KC_C), LGUI(KC_V), KC_LEFT,  _______,  _______, KC_DOWN,           XXXXXXX, KC_COMMA,KC_DOT,  JP_SLSH, JP_YEN, \
                            _______,     _______,    _______,    _______,  _______,  _______, _______,           _______, _______, _______\
)
